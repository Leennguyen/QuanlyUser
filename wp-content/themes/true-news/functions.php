...
if ( !function_exists('true_news_admin_scripts') ):
    ...
    function true_news_admin_scripts(){
        ...
        wp_enqueue_media();
        wp_enqueue_script('true-news-admin', get_template_directory_uri() . '/assets/js/admin.js', array('jquery'), '1.0.0', true);
	    ...
	    wp_localize_script( 
	        ...
	        array(
	            'ajax_url'   => esc_url( admin_url( 'admin-ajax.php' ) ),
	            'ajax_nonce' => $ajax_nonce,
	            'active' => esc_html__('Active','true-news'),
		        'deactivate' => esc_html__('Deactivate','true-news'),
	         )
	    );
...
if( class_exists('Demo_Import_Kit_Class') ):
    ...
    add_filter('themeinwp_enable_demo_import_compatiblity','true_news_demo_import_filter_apply');

    if( !function_exists('true_news_demo_import_filter_apply') ):

        function true_news_demo_import_filter_apply(){
