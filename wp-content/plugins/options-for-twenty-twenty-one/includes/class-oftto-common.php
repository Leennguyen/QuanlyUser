...
if (!class_exists('ofttoCommon')) {
	...
	class ofttoCommon {
        ...
        public static $plugin_name = 'Options for Twenty Twenty-One';
        public static $plugin_prefix = 'oftto';
        public static $plugin_text_domain = 'options-for-twenty-twenty-one';
        public static $plugin_premium_class = 'options_for_twenty_twenty_one_premium_class';
        public static $plugin_trial = true;
        public static $plugin_upgrade = true;

		public static function plugin_version() {

            return options_for_twenty_twenty_one_class::$version;
        ...
        public static function control_upgrade_text() {
            ...
            $upgrade_text = '<a href="' . self::upgrade_link() . '" title="' . addslashes(sprintf(__('Upgrade now to %s Premium', self::$plugin_text_domain), self::$plugin_name)) . '">' . sprintf(__('Upgrade now to %s Premium', self::$plugin_text_domain), self::$plugin_name) . '</a>';
        ...
        public static function plugin_action_links($settings_link, $premium = false) {
			...
			$settings_links[] = '<a href="' . esc_url($settings_link) . '" title="' . addslashes(__('Settings', self::$plugin_text_domain)) . '">' . __('Settings', self::$plugin_text_domain) . '</a>';
            ...
            if (!get_option(self::$plugin_prefix . '_purchased')) {
                ...
                if ($premium) {
                    ...
                    if (self::$plugin_upgrade) {
                        ...
                        $settings_links[] = '<a href="' . self::upgrade_link() . '" title="' . addslashes(sprintf(__('Buy %s Premium', self::$plugin_text_domain), self::$plugin_name)) . '" style="color: orange; font-weight: bold;">' . __('Buy Now', self::$plugin_text_domain) . '</a>';

                    } else {

                        $settings_links[] = '<a href="' . self::upgrade_link() . '" title="' . addslashes(sprintf(__('Buy %s', self::$plugin_text_domain), self::$plugin_name)) . '" style="color: orange; font-weight: bold;">' . __('Buy Now', self::$plugin_text_domain) . '</a>';

                    }

                } else {

                    $settings_links[] = '<a href="' . self::upgrade_link() . '" title="' . addslashes((self::$plugin_premium_class ? sprintf(__('Upgrade now to %s Premium', self::$plugin_text_domain), self::$plugin_name) : sprintf(__('Contribute to %s Premium', self::$plugin_text_domain), self::$plugin_name))) . '" style="color: orange; font-weight: bold;">' . (self::$plugin_premium_class ? __('Upgrade', self::$plugin_text_domain) : __('Support Us', self::$plugin_text_domain)) . '</a>';

                }

                if ($premium) {

                    $settings_links[] = '<a href="' . wp_nonce_url('?activate-' . self::$plugin_prefix . '=true', self::$plugin_prefix . '_activate') . '" id="' . self::$plugin_prefix . '_activate_upgrade" title="' . addslashes(__('Activate Purchase', self::$plugin_text_domain)) . '" onclick="jQuery(this).append(&#39; <img src=&#34;/wp-admin/images/loading.gif&#34; style=&#34;float: none; width: auto; height: auto;&#34; />&#39;); setTimeout(function(){document.getElementById(\'' . self::$plugin_prefix . '_activate_upgrade\').removeAttribute(\'href\');},1); return true;">' . __('Activate Purchase', self::$plugin_text_domain) . '</a>';

                } elseif (self::$plugin_trial && !is_plugin_active(self::$plugin_text_domain . '-premium/' . self::$plugin_text_domain . '-premium.php')) {

                    $settings_links[] = '<a href="' . self::premium_link() . '" title="' . addslashes(sprintf(__('Trial %s Premium', self::$plugin_text_domain), self::$plugin_name)) . ' for 7 days">' . __('Download Trial', self::$plugin_text_domain) . '</a>';
        ...
        public static function plugin_row_meta($plugin_meta, $plugin_file, $plugin_data, $status) {
            ...
            if ($plugin_file === self::$plugin_text_domain . '/' . self::$plugin_text_domain . '.php') {
                ...
                $plugin_meta[] = '<a href="' . self::support_url() . '" title="' . __('Problems? We are here to help!', self::$plugin_text_domain) . '" style="color: orange; font-weight: bold;">' . __('Need help?', self::$plugin_text_domain) . '</a>';
                $plugin_meta[] = '<a href="https://wordpress.org/support/plugin/' . self::$plugin_text_domain . '/reviews/#new-post" title="' . addslashes(sprintf(__('If you like %s, please leave a review!', self::$plugin_text_domain), self::$plugin_name)) . '">' . __('Review plugin', self::$plugin_text_domain) . '</a>';
...
<?php _e('In order to use the premium features, you need to install the premium version of the plugin ...', self::$plugin_text_domain); ?></p>

<p><a href="<?php echo self::premium_link(); ?>" title="<?php echo addslashes(sprintf(__('Download %s Premium', self::$plugin_text_domain), self::$plugin_name)); ?>" class="button-primary"><?php printf(__('Download %s Premium', self::$plugin_text_domain), self::$plugin_name); ?></a></p>
...
<a href="<?php echo self::premium_link(); ?>" title="<?php echo addslashes(sprintf(__('Try %s Premium', self::$plugin_text_domain), self::$plugin_name)); ?>" class="button-primary"><?php printf(__('Trial %s Premium for 7 days', self::$plugin_text_domain), self::$plugin_name); ?></a>
...
?>
<a href="<?php echo self::upgrade_link(); ?>" title="<?php echo addslashes(sprintf(__('Upgrade now to %s Premium', self::$plugin_text_domain), self::$plugin_name)); ?>" class="button-primary"><?php printf(__('Upgrade now to %s Premium', self::$plugin_text_domain), self::$plugin_name); ?></a></p>
...
?>
<p><?php _e('Funding plugins like this one with small financial contributions is essential to pay the developers to continue to do what they do. Please take a moment to give a small amount ...', self::$plugin_text_domain); ?></p>
<p><a href="<?php echo self::upgrade_link(); ?>" title="<?php echo addslashes(sprintf(__('Contribute to %s', self::$plugin_text_domain), self::$plugin_name)); ?>" class="button-primary"><?php printf(__('Contribute to %s', self::$plugin_text_domain), self::$plugin_name); ?></a> <a href="#" id="<?php echo self::$plugin_prefix; ?>-already-paid" title="<?php echo addslashes(__('Aleady Contributed!', self::$plugin_text_domain)); ?>" class="button-primary"><?php _e('Aleady Contributed!', self::$plugin_text_domain); ?></a></p>
</div>
...
<?php
        ...
        public static function is_theme_being_used($themes) {
            ...
            if (
                ...
                $child = wp_get_theme(sanitize_file_name($_GET['customize_theme']));
            ...
            if (
                ...
                if (isset($_GET['theme'])) {
                    ...
                    $child = wp_get_theme(sanitize_file_name($_GET['theme']));

                } else {

                    $child = wp_get_theme(sanitize_file_name($_POST['customize_theme']));
